...
if [[ ! -e ${HOME}/.noElementsEnvLoginScript ]]; then
  ...
  if [[ -r ${my_own_prefix2}/bin/ElementsEnv_config.sh ]]; then
    confscr=${my_own_prefix2}/bin/ElementsEnv_config.sh
  else
  ...
  if [[ -n "$E_BANNER" ]]; then
    ...
    if [[ ! -n "$ELEMENTSENV_POST_DONE" ]]; then
      if [[ -n "$ELEMENTSENV_POST_SCRIPT" ]]; then
        if [[ -r ${my_own_prefix2}/bin/${ELEMENTSENV_POST_SCRIPT}.sh ]]; then
          epostscr=${my_own_prefix2}/bin/${ELEMENTSENV_POST_SCRIPT}.sh
        else
