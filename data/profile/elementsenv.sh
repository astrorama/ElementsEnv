...
if [[ ! -e ${HOME}/.noElementsEnvLoginScript ]]; then
  ...
  if [[ -r ${my_own_prefix4}/bin/ElementsEnv_config.sh ]]; then
    confscr=${my_own_prefix4}/bin/ElementsEnv_config.sh
  else
  ...
  if [[ -r ${my_own_prefix4}/bin/ElementsEnv_group_setup.sh ]]; then
    ...
    shellscr=`/usr/bin/which ElementsEnv_group_setup.sh`
  fi
  if  [[ -e ${shellscr} ]]; then
     . ${shellscr} "$@"
  fi
  ...
  if ( (shopt -q login_shell || [[ -o login ]]) && ( [[ $- == *i* ]] || -o interactive) ) 2> /dev/null ; then
    if [[ -r ${my_own_prefix4}/bin/ElementsEnv_group_login.sh ]]; then
      loginscr=${my_own_prefix4}/bin/ElementsEnv_group_login.sh
    else
      loginscr=`/usr/bin/which ElementsEnv_group_login.sh`
    fi
    if [[ -e ${loginscr} ]]; then
       . ${loginscr} "$@"
    fi
